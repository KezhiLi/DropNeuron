# -*- coding: utf-8 -*-
"""
Created on Mon Jul  4 14:12:06 2016
...
#0.0001 0  0.0005  0.5 0.01
#lambda_l1 = float(0.0001)
#lambda_l2 = float(0)
#lambda_dropneuron = float(0.0005)
#keep_prob = float(0.5)  # keep_prob \in (0, 1]
...
x_flg = 0
x_ind = np.array(range(0,n_input))
...
def prune(x):
    ...
    ind_r = np.unique(np.nonzero(y_prune)[0])
    ind_c = np.unique(np.nonzero(y_prune)[1])
    y_prune_small = y_prune[ind_r][:,ind_c]
    
    ind_r_cpl = np.delete(np.arange(y_prune.shape[0]),ind_r)
    if len(ind_r_cpl)>1:
        ind_r = np.append(ind_r,random.sample(ind_r_cpl, int(round(0.3*len(ind_r_cpl)))))
        ind_r = np.sort(ind_r)
    ind_c_cpl = np.delete(np.arange(y_prune.shape[1]),ind_c)    
    if len(ind_c_cpl)>1:
        ind_c = np.append(ind_c,random.sample(ind_c_cpl, int(round(0.3*len(ind_c_cpl)))))
        ind_c = np.sort(ind_c)
        
    return y_noprune, y_prune, y_prune_small, ind_r, ind_c
...
checkpoint_dir = "/tmp/300-100_tmp1.ckpt"
div = 4
...
for ii in range(1,div+1):
    ...
    with tf.Session() as sess:
       ...
       #     if x_flg:
       #         pred = model(x, W, biases)
      #      if col_redun_fc1>0 or row_redun_out>0:
            init = tf.initialize_all_variables()
            sess.run(init)
            saver.restore(sess, checkpoint_dir)
            
        # Training cycle
        for epoch in range(training_epochs/div):
            avg_loss = 0.
        ...
        w_fc1_, w_fc1, w_fc1_sm, w_fc1_r, w_fc1_c = prune(W['fc1'])
        w_fc2_, w_fc2, w_fc2_sm, w_fc2_r, w_fc2_c = prune(W['fc2'])
        w_out_, w_out, w_out_sm, w_out_r, w_out_c = prune(W['out'])


        #sess.run(W_prune) 
        if ii == div:
            
            W['fc1'].assign(w_fc1, use_locking=False)
            W['fc2'].assign(w_fc2, use_locking=False)
            W['out'].assign(w_out, use_locking=False)
        ...
        row_redun_fc1 = w_fc1.shape[0]-len(w_fc1_r)
        col_redun_fc1 = w_fc1.shape[1]-len(w_fc1_c)
        row_redun_fc2 = w_fc2.shape[0]-len(w_fc2_r)
        col_redun_fc2 = w_fc2.shape[1]-len(w_fc2_c)
        row_redun_out = w_out.shape[0]-len(w_out_r)
        
#        # update matrix, reduce the dimentions of connections
#        if row_redun_fc1>0:
#            current_x_len = len(w_fc1_r)
#        #    X['x'].assign(batch_x[:,x_ind], use_locking=False) 
#        #    X = { 'x': tf.placeholder("float", [None, None]) }
#        #    X['x'].assign(batch_x[:,x_ind], use_locking=False) 
#            x_flg = 1
#            x_ind = x_ind[w_fc1_r]
#            W = {
#                'fc1': tf.Variable(w_fc1[w_fc1_r,:]),
#                'fc2': W['fc2'],
#                'out': W['out']
#            }
#            W['fc1'].assign(w_fc1[w_fc1_r,:], use_locking=False)
            
        if col_redun_fc1>0:           
            W = {
                'fc1': tf.Variable(w_fc1[:,w_fc1_c]),
                'fc2': tf.Variable(w_fc2[w_fc1_c,:]),
                'out': W['out']
            }
            W['fc1'].assign(w_fc1[:,w_fc1_c], use_locking=False)
            W['fc2'].assign(w_fc2[w_fc1_c,:], use_locking=False)  
            
            biases_fc1 = sess.run(biases['fc1'])[w_fc1_c]
            biases = {
                'fc1': tf.Variable(biases_fc1),
                'fc2': biases['fc2'],
                'out': biases['out']
            }
            biases['fc1'].assign(biases_fc1, use_locking=False)   
            
        elif row_redun_fc2>0:
            W = {
                'fc1': tf.Variable(w_fc1[:,w_fc2_r]),
                'fc2': tf.Variable(w_fc2[w_fc2_r,:]),
                'out': W['out']
            }
            W['fc1'].assign(w_fc1[:,w_fc2_r], use_locking=False)
            W['fc2'].assign(w_fc2[w_fc2_r,:], use_locking=False)
            
            biases_fc1 = sess.run(biases['fc1'])[w_fc2_r]
            biases = {
                'fc1': tf.Variable(biases_fc1),
                'fc2': biases['fc2'],
                'out': biases['out']
        ...
        if col_redun_fc2>0:
            W = {
                'fc1': W['fc1'],
                'fc2': tf.Variable(w_fc2[:,w_fc2_c]),
                'out': tf.Variable(w_out[w_fc2_c,:])
            }
            W['fc2'].assign(w_fc2[:,w_fc2_c], use_locking=False)
            W['out'].assign(w_out[w_fc2_c,:], use_locking=False)  
            
            biases_fc2 = sess.run(biases['fc2'])[w_fc2_c]
            biases = {
                'fc1': biases['fc1'],
                'fc2': tf.Variable(biases_fc2),
                'out': biases['out']
            }
            biases['fc2'].assign(biases_fc2, use_locking=False)             

        elif row_redun_out>0:
            W = {
                'fc1': W['fc1'],
                'fc2': tf.Variable(w_fc2[:,w_out_r]),
                'out': tf.Variable(w_out[w_out_r,:])
            }
            W['fc2'].assign(w_fc2[:,w_out_r], use_locking=False)
            W['out'].assign(w_out[w_out_r,:], use_locking=False)
            
            biases_fc2 = sess.run(biases['fc2'])[w_out_r]
            biases = {
                'fc1': biases['fc1'],
                'fc2': tf.Variable(biases_fc2),
                'out': biases['out']
